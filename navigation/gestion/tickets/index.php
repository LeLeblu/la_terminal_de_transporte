...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $stmt = $cn->prepare("INSERT INTO tickets
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
  $stmt->bind_param("issssssiissss", $empresa_id, $tipo, $fecha, $horario, $origen, $destino,
    $sillas, $cantidad, $costo, $total, $nombre, $cedula, $contacto);
  ...
  <section class="section">
    ...
    <div class="results" style="grid-template-columns:1fr;">
      <table class="routes-table" style="width:100%;">
        <thead>
          <tr>
            <th>#</th><th>Empresa</th><th>Tipo</th><th>Ruta</th><th>Fecha</th><th>Hora</th>
            <th>Sillas</th><th>Cant</th><th>Total</th><th>Cliente</th><th>Acciones</th>
          </tr>
        </thead>
        <tbody>
        <?php while($r=$tickets->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              ...
              <a class="menu-link" href="../../../dataBase/eliminar_ticket.php?id=<?php echo $r['id']; ?>"
    ...
    <form method="post" class="grid-2">
      ...
      <label>Tipo de vehículo
        <select name="tipo_vehiculo">
          <option>TAXI</option><option>AEROVAN</option>
          <option>BUS_1PISO</option><option>BUS_2PISOS</option>
        </select>
      </label>
